 ...
 * 
 * Сценарий: Необходимо хранить данные зарегистрированных пользователей.
 * 
...
-- Ещё раз добавляем пользователя, счётчик присваивает id = 3
INSERT INTO users VALUES (DEFAULT, 'Vasya', 'Vasilkov', 'vasya@mail.com', '89212023334', DEFAULT, DEFAULT);
...
/* Диапазоны значений для типов данных 

INT от -2147483648 до 2147483647
INT UNSIGNED от 0 до 4294967295

BIGINT от -9 223 372 036 854 775 808 до 9 223 372 036 854 775 807
BIGINT UNSIGNED 0 до 18 446 744 073 709 551 615
...
INSERT INTO messages VALUES (DEFAULT, 1, 3, 'Hi!', 1, DEFAULT, DEFAULT); -- сообщение от Пети к Васе номер 1
INSERT INTO messages VALUES (DEFAULT, 1, 3, 'Vasya!', 1, DEFAULT, DEFAULT); -- сообщение от Пети к Васе номер 2
INSERT INTO messages VALUES (DEFAULT, 3, 1, 'Hi, Petya', 1, DEFAULT, DEFAULT); -- сообщение от Пети к Васе номер 2
...
/* Создаем таблицу для хранения платежных данных и карт пользователя
 * card_number - номер карты
 * card_date_valid - срок использования
 * card_username - имя владельца на карте
 * card_pin - пин код
...
/*Создаем таблицу для поиска работы с информацией для работодателя
 * work_experience - опыт работы - текстовое поле для описания предыдущего опыта работы
 * speciality - специальность - желаемая специальность
 * posk_status - статус поиска: в активном поиске, нашел работу, подработка...
 */
...
/*Создаем таблицу для учета игр пользователя
*game_user - название текущей игры пользователя
*game_level - уровень достижений в игре
*game_date_at - дата последнего посещения игры
